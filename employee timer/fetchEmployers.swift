...
//  Copyright © 2018 אורי עינת. All rights reserved.
        ...
        self.dogItem = String(describing: snapshot.childSnapshot(forPath: "fName").value!) as String!
          ...
          if self.profileImageUrl == "" {
            ...
            let employerToAdd:employerStruct = employerStruct( accountName:"\(self.dogItem) \(self.employerItem)" , employerRef: self.employerIdArray2[iIndex] as! String, sesQty: self.sesQtyTemp!, activeAccount: (snapshot.childSnapshot(forPath: "fActive").value as! Bool), lastDocAccount: self.lastDocumentItem, accountImageUrl:self.profileImageUrl )
